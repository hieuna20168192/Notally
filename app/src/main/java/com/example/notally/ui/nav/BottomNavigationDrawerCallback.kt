...
/**
 * A [BottomSheetBehavior.BottomSheetCallback] which helps break apart clients who would like to react
 * to changed in either the bottom sheet's slide offset or state. Clients can dynamically add or remove
 * [OnSlideAction]s or [OnStateChangedAction]s which be run when the sheet's slideOffset or state are changed.
 *
 * This callback's behaviour differs slightly in that the slideOffset passed to [OnSlideAction]s in [onSlide] is corrected
 * to guarantee that the offset 0.0 <i>always<i> be exactly at the [BottomSheetBehavior.STATE_HALF_EXPANDED] state.
 */
class BottomNavigationDrawerCallback : BottomSheetBehavior.BottomSheetCallback() {
    ...
    override fun onSlide(sheet: View, slideOffset: Float) {
        if (halfExpandedSlideOffset == Float.MAX_VALUE)
            calculateInitialHalfExpandSlideOffset(sheet)
     ...
     * this method is used to calculate the initial value manually so we can smoothly normalize slideOffset values
     * received between -1 and 1.
     */
    private fun calculateInitialHalfExpandSlideOffset(sheet: View) {
        val parent = sheet.parent as CoordinatorLayout
        val behaviour = BottomSheetBehavior.from(sheet)

        val halfExpandedOffset = parent.height * (1 - behaviour.halfExpandedRatio)
